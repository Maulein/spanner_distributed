...
public:
...

    {
        ...
        // d, 1., 1.e-5));
	std::cout<<"rmat_iterator"<<std::endl;
        if (permute_vertices)
...
public:
	...
	std::cout<<"sorted_iterator"<<std::endl;
        std::vector< vertices_size_type > vertexPermutation;
        if (permute_vertices)
            generate_permutation_vector(gen, vertexPermutation, n);
...
public:
...

    {
        ...
        this->gen.reset(new uniform_01< RandomGenerator >(gen));
	std::cout<<"unique_iterator"<<std::endl;
...
public:
	...
	std::cout<<"sorted_unique_iterator"<<std::endl;
        std::vector< vertices_size_type > vertexPermutation;
        if (permute_vertices)
            generate_permutation_vector(gen, vertexPermutation, n);
...
#include BOOST_GRAPH_MPI_INCLUDE("dist_rmat_graph_generator.hpp")
