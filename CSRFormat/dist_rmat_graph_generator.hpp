...
//	  std::cout<<"scalable_rmat_iterator"<<std::endl;
          ...
          edges_size_type local_edges_same = 0;
         {
	  ...
	  do {
          ...
          //       Asking for a value too close to n^2 edges may as well
              edges_size_type locals = all_reduce(pg, local_edges, boost::parallel::sum<vertices_size_type>());
              edges_size_type locals_same = all_reduce(pg, local_edges_same, boost::parallel::sum<vertices_size_type>());
	  if(id == 0)
		  std::cout<<" Generated in Map | local edges "<<locals<<" | same id local edges "<<locals_same<<std::endl;
	  ...
	  } //end scope of std::map
          values.reserve(local_edges + local_edges_same);
	  for (auto vec_i = values.rbegin() ; vec_i != values.rend() ; vec_i++)
	  {
	      auto edge = *vec_i;
	      auto u = edge.first;
	      auto v = edge.second;
	      if(distrib(u) == id && distrib(v) == id)
		{
          ...
          values.pop_back();
	  if(id == 0)
		  std::cout<<" Map to vector"<<std::endl;
