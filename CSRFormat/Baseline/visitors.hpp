...
template < class ClusterMap, class ParentMap, class Tag >
struct cluster_recorder
: public base_visitor< cluster_recorder< ClusterMap, ParentMap, Tag > >
{
    typedef Tag event_filter;
    cluster_recorder(ClusterMap ca, ParentMap pa) : m_cluster(ca), m_parent(pa) {}
...
template < class ClusterMap, class ParentMap, class Tag >
cluster_recorder< ClusterMap, ParentMap, Tag > record_clusters(ClusterMap ca, ParentMap pa,  Tag)
{
    return cluster_recorder< ClusterMap, ParentMap, Tag >(ca, pa);
}
...
template < class ParentMap, class PredMap, class SpannerMap, class DistanceMap, class Map, class Tag >
struct parent_recorder
: public base_visitor< parent_recorder< ParentMap, PredMap, SpannerMap, DistanceMap, Map, Tag > >
{
    typedef Tag event_filter;
    parent_recorder(ParentMap pa, PredMap pra, SpannerMap sa, DistanceMap da, Map ma) : m_parent(pa), m_pred(pra), m_spanner(sa), m_distance(da), m_map(ma){}
    template < class Vertex, class Graph > void operator()(Vertex v, const Graph& g)
    {
...
template < class ParentMap, class PredMap, class SpannerMap, class DistanceMap, class Map, class Tag >
parent_recorder< ParentMap, PredMap, SpannerMap, DistanceMap, Map, Tag > record_parents(ParentMap pa, PredMap pra, SpannerMap sa, DistanceMap da, Map ma, Tag)
{
    return parent_recorder< ParentMap, PredMap, SpannerMap,  DistanceMap, Map, Tag >(pa, pra, sa, da, ma);
}
//========================================================================
// time_stamper
