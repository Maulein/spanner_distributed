...
template < class ClusterMap, class ParentMap, class InvalidMap, class Tag >
struct cluster_recorder
: public base_visitor< cluster_recorder< ClusterMap, ParentMap, InvalidMap, Tag > >
{
    typedef Tag event_filter;
    cluster_recorder(ClusterMap ca, ParentMap pa, InvalidMap ia) : m_cluster(ca), m_parent(pa), m_invalid(ia) {}
    template < class Vertex, class Graph > void operator()(Vertex v, const Graph& g)
    {
  	auto v_invalid = get(m_invalid,v);
	typename graph_traits< Graph >::vertex_descriptor p = get(m_parent,v);
  	if (p != v)
        	put(m_cluster, v, get(m_cluster, p));
	else
		put(m_cluster, v, v);
    }
    ClusterMap m_cluster;
    ParentMap m_parent;
    InvalidMap m_invalid;
};
template < class ClusterMap, class ParentMap, class InvalidMap, class Tag >
cluster_recorder< ClusterMap, ParentMap, InvalidMap, Tag > record_clusters(ClusterMap ca, ParentMap pa, InvalidMap ia, Tag)
{
    return cluster_recorder< ClusterMap, ParentMap, InvalidMap, Tag >(ca, pa, ia);
}

//========================================================================
// pred_nums_initializer

template < class PredecessorMap, class Map, class Tag >
struct pred_nums_initializer
: public base_visitor< pred_nums_initializer< PredecessorMap, Map, Tag > >
{
    typedef Tag event_filter;
    pred_nums_initializer(PredecessorMap pa, Map ma) : m_predecessor(pa), m_map(ma) {}
    template < class Vertex, class Graph > void operator()(Vertex v, const Graph& g)
    {
	// Added for Tie breaking FGV    
	//size_t num_v = g.distribution().global( boost::graph::distributed::mpi_process_group::owner(v), local(v));
	size_t num_v = get(m_map,v);
        put(m_predecessor, v, num_v);
...
template < class PredecessorMap, class Map, class Tag >
pred_nums_initializer< PredecessorMap, Map, Tag > initialize_pred_nums(
    PredecessorMap pa, Map ma, Tag)
{
    return pred_nums_initializer< PredecessorMap, Map, Tag >(pa, ma);
}
//========================================================================
// pred_nums_recorder

template < class PredecessorMap, class Map, class Tag >
struct pred_nums_recorder
: public base_visitor< pred_nums_recorder< PredecessorMap, Map, Tag > >
{
    typedef Tag event_filter;
    pred_nums_recorder(PredecessorMap pa, Map ma) : m_predecessor(pa), m_map(ma) {}
    template < class Edge, class Graph > void operator()(Edge e, const Graph& g)
    {
	// Added for Tie breaking FGV    
//	auto src = get(m_predecessor,source(e,g));
//	size_t num_src = g.distribution().global( boost::graph::distributed::mpi_process_group::owner(src), local(src));
	size_t num_src = get(m_map, source(e,g));
        put(m_predecessor, target(e, g), num_src);
...
template < class PredecessorMap, class Map, class Tag >
pred_nums_recorder< PredecessorMap, Map, Tag > record_pred_nums(
    PredecessorMap pa, Map ma, Tag)
{
    return pred_nums_recorder< PredecessorMap, Map, Tag >(pa, ma);
}

//========================================================================
// parent_recorder

template < class ParentMap, class PredMap, class SpannerMap, class DistanceMap, class Map, class InvalidMap, class Tag >
struct parent_recorder
: public base_visitor< parent_recorder< ParentMap, PredMap, SpannerMap, DistanceMap, Map, InvalidMap, Tag > >
{
    typedef Tag event_filter;
    parent_recorder(ParentMap pa, PredMap pra, SpannerMap sa, DistanceMap da, Map ma, InvalidMap ia) : m_parent(pa), m_pred(pra), m_spanner(sa), m_distance(da), m_map(ma), m_invalid(ia){}
    template < class Vertex, class Graph > void operator()(Vertex v, const Graph& g)
    {
        auto pred = get(m_pred, v); //numeric
	auto pred_vd = vertex(pred, g) ; //vertex descriptor
	put(m_parent, v, pred_vd);  // m_pred is numeric convert to vertex descriptor to maintain minimum parent id as parent - FGV
  	auto v_invalid = get(m_invalid,v);
        // Add Tree Edge to Spanner
	auto parent = pred_vd; // get(m_parent, v);
	//if( parent != v_invalid && parent != v) {
	if(parent != v) {
		auto tree_edge = edge(v, parent, g);
		if(tree_edge.second)
		{ 
			put(m_spanner, tree_edge.first, true);
		}
	}
    }
    ParentMap m_parent;
...
template < class ParentMap, class PredMap, class SpannerMap, class DistanceMap, class Map, class InvalidMap, class Tag >
parent_recorder< ParentMap, PredMap, SpannerMap, DistanceMap, Map, InvalidMap, Tag > record_parents(ParentMap pa, PredMap pra, SpannerMap sa, DistanceMap da, Map ma, InvalidMap ia,Tag)
{
    return parent_recorder< ParentMap, PredMap, SpannerMap,  DistanceMap, Map, InvalidMap, Tag >(pa, pra, sa, da, ma, ia);
}
//========================================================================
// time_stamper
