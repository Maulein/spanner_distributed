...
//#include "bfs1hop_distributed_forspanner.hpp"

#include <boost/unordered/unordered_flat_map.hpp>
#include <iostream>
//typedef boost::unordered_flat_map<size_t, size_t, boost::hash<size_t>, std::equal_to<size_t>,std::allocator<std::pair<const size_t,size_t>>> MyMap;
...
struct spanner_intra_t {
  ...
  enum vertex_center_t { vertex_center };
  ...
  enum vertex_invalid_t { vertex_invalid};
  ...
  BOOST_INSTALL_PROPERTY(vertex, center);
  ...
  BOOST_INSTALL_PROPERTY(vertex, invalid);
...
ldd(Graph &g, int k, SpannerGraph &sg,std::ofstream& f_log ){
 ...
 //  Internal property map to store predecessor
  ...
  // Internal property map to store final parent
  property_map<Graph,  invalid_t>::type
    invalid = get(invalid_t(), g);
  ...
  // Internal property map to store whether edge is part of spanner or not (Intercluster)
  ...
  breadth_first_search
    (g, start,
     visitor(make_bfs_visitor(std::make_pair(put_property(distance, 0, on_initialize_vertex()), //Initialize distance to 0
				...
				std::make_pair(record_parents(parent, pred_numeric, spanner_intra, distance, map, invalid, on_examine_vertex()),  
		     		record_clusters(cluster, parent, invalid, on_examine_vertex())))))))));
  ...
  BGL_FORALL_EDGES(e, g, Graph) {
	  ...
	  request(spanner_inter, e);
  }
		  
  synchronize(spanner_intra); 
  synchronize(spanner_inter); 
