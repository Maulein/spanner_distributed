    ...
    SourceIterator sources_end, Buffer& Q,Buffer& Q1, BFSVisitor vis, ColorMap color)
{
  ...
  auto		 center = get(boost::vertex_root, g);
  auto		 distance = get(boost::vertex_distance, g);
	...
	// Swap the current frontier and next frontier for the next round
      	if (count%2) {	
	    current_frontier = &Q1;
	    next_frontier = &Q;
	}
    ...
    BGL_FORALL_VERTICES_T(v, g, IncidenceGraph)
    {
	  ...
	  if(get(distance,v) > 2 )
		  std::cout<<"WRONG DISTANCE\n";
	  
	  if(get(distance,v) <= 2 )
		visited ++;
    }
   size_t global_unvisited = boost::parallel::all_reduce(pg, unvisited, std::plus<size_t>());

   size_t global_visited = boost::parallel::all_reduce(pg, num_visited, std::plus<size_t>());
   int last_round = k-1;
    int  r = k-1; 
    ...
    // BFS
    size_t num_to_add = 0, num_to_add_total = 0, num_added = 0;
    size_t index = 0;
    ...
    size_t error_vertex = 0;
    synchronize(color);
    while(ldd_round <= last_round) //ldd_round initilized to 0 and last_round = k-1  
    {	
	int start = shifts[ldd_round];
    	int end = shifts[ldd_round + 1];
    	num_to_add_total = end - start;
	//std::cout<<process_id(pg)<<" process, round "<<ldd_round<<" total to be added centers "<<num_to_add_total<<"\n";
	...
	else if  ( num_to_add_total > 0 )  { // Number of Processors is less than the number of vertices to be picked as centers
...
// FGV - Tie Breaking: Set the lowest ID neighbor as parent for each of the vertices in next frontier - visitor.hpp
	...
	// Swap the current frontier and next frontier for the next round
    	...
    	if (ldd_round == (last_round + 1) && !(current_frontier->empty())) 
	    std::cout<<" WHY is this not empty \n";
    BGL_FORALL_VERTICES_T(v, g, IncidenceGraph)
    {
	  if(get(distance,v) == 3 )
		  std::cout<<" WRONG DISTANCE"<<get(distance,v)<<" - After LDD\n";
	
    }
    }
    BGL_FORALL_VERTICES_T(v, g, IncidenceGraph)
    {
	  if(get(color,v) == Color::white() && out_degree(v,g)!=0)
		error_vertex++;
	
    }
   	size_t global_error = boost::parallel::all_reduce(pg, error_vertex, std::plus<size_t>());
} //breadth_first_visit
