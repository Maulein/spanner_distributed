...
//#define RMAT
#define ReadDirectFromFile
//#define graph500
#define CompactSpanner
//#define heuristic3 //Multiple Independent Random Walkers
...
typedef adjacency_list<vecS,
                         ...
                         undirectedS>::vertex_descriptor vertex_descriptor;
...
typedef adjacency_list<vecS,
			 ...
			 property<vertex_index_t,size_t,
				...
				property<parent_HCIS_t, vertex_descriptor,
			...
			// Edge Properties
			property<spanner_inter_t, bool,
				property<spanner_intra_t,bool, 
				property<picked_edge_t, bool>>>, no_property, listS>
    Graph;
typedef adjacency_list<vecS,
                         distributedS<mpi_process_group, vecS>,
                         undirectedS,
...
void 
calc_coverage(Graph &g, int k, int round){		 
...
void find_subgraph(Graph &g, float length, float percentage, std::ofstream& f_log, float extend, size_t& size_sub)
{
	...
	property_map<Graph,  frontier_t>::type
  	...
  	property_map<Graph,  picked_edge_t>::type
    		picked_edge = get(picked_edge_t(), g);
	property_map<Graph,  vertex_out_degree_t>::type
  		  deg = get(vertex_out_degree, g);
  	
	picked.set_consistency_model(boost::parallel::cm_bidirectional);
  	picked.set_max_ghost_cells(0);
  	put_queue.set_consistency_model(boost::parallel::cm_bidirectional);
  	put_queue.set_max_ghost_cells(0);
  	deg.set_max_ghost_cells(0);
	...
	auto pro_id = process_id(process_group(g));
    	...
    	int global_index_begin = g.distribution().global(owner(v_first), local(v_first));
	...
	BGL_FORALL_VERTICES(v, g, Graph) { 
		...
		put(put_queue,v,false);
    		put(map, v, g.distribution().global(owner(v), local(v))); 
...
void create_subgraph(Graph &g, Graph &sub_g, int k) {
	auto pro_id = process_id(process_group(g));
 	typedef graph_traits< Graph > GTraits;
    	typedef typename GTraits::vertex_descriptor Vertex;

  	Vertex v_first_g= (vertices(g).first[0]);
    	int global_index_begin_g = g.distribution().global(owner(v_first_g), local(v_first_g));

  	Vertex v_first = (vertices(sub_g).first[0]);
    	int global_index_begin = sub_g.distribution().global(owner(v_first), local(v_first));	
	property_map<Graph,  vertex_degree_t>::type
  		 picked = get(vertex_degree, g);
	...
	BGL_FORALL_VERTICES(v, sub_g, Graph) { 
	...
	BGL_FORALL_VERTICES(v,g,Graph) {
		BGL_FORALL_OUTEDGES(v,e,g,Graph) {
		   if(get(picked_edge, e) ==  true)	{
			...
			size_t target_g_num = sub_g.distribution().global(owner(target_g), local(target_g));
			Vertex src = vertex(src_g_num, sub_g);
			Vertex target =  vertex( target_g_num, sub_g);
...
void map_back(Graph &g, Graph &sub_g) {
	...
	BGL_FORALL_VERTICES(v, g, Graph) {
		put(root_g, v, false);
    		put(map_g, v, g.distribution().global(owner(v), local(v))); 
	}
	BGL_FORALL_VERTICES(v, sub_g, Graph) {
		if(get(root, v) == true) {
...
cleanup(Graph &g) {
	...
	property_map<Graph,  inMIS_t>::type
  		 inMIS = get(inMIS_t(), g);
	property_map<Graph,  propagate_t>::type
  		 propagate = get(propagate_t(), g);
	property_map<Graph,  cluster_t>::type
  		 cluster = get(cluster_t(), g);
	property_map<Graph,  active_t>::type
  		 active = get(active_t(), g);
	property_map<Graph,  parent_HCIS_t>::type
  		 parent_HCIS = get(parent_HCIS_t(), g);
	property_map<Graph,  frontier_t>::type
  		 frontier = get(frontier_t(), g);
	property_map<Graph,  more_t>::type
  		 more = get(more_t(), g);
	...
	property_map<Graph,  map_t>::type
  		 map = get(map_t(), g);
  property_map<Graph,  spanner_inter_t>::type
    spanner_inter = get(spanner_inter_t(), g);
...
void 
HCIS(Graph &g, int k,  std::ofstream& f_log, int iterations ) {
	property_map<Graph,  vertex_out_degree_t>::type
 		 deg = get(vertex_out_degree, g);
	property_map<Graph,  vertex_degree_t>::type
 		 coverage = get(vertex_degree, g);
...
#if defined CompactSpanner && defined RMAT 
iterations = 1;
#endif
...
ldd(Graph &g, int k, SpannerGraph &sg, std::ofstream& f_log ){
  ...
  spanner_inter.set_consistency_model(boost::parallel::cm_bidirectional);
  spanner_intra.set_consistency_model(boost::parallel::cm_bidirectional);
  
  spanner_inter.set_max_ghost_cells(0);
  spanner_intra.set_max_ghost_cells(0);
  ...
  BGL_FORALL_VERTICES(v, g, Graph){
    BGL_FORALL_OUTEDGES(v, e, g, Graph){
      ...
      size_t src_num =  g.distribution().global(owner(src), local(src));
      size_t ngh_num =  g.distribution().global(owner(ngh), local(ngh)); 
      size_t c_src_num = g.distribution().global(owner(c_src), local(c_src));      
      size_t c_ngh_num = g.distribution().global(owner(c_ngh), local(c_ngh));     
...
main(int argc, char* argv[]){
...
#ifdef ReadDirectFromFile
        const char* filename = "RMAT_Graph_test1.txt";
        if (argc > 1)
                filename = argv[1];
        std::string line;
	...
	while(myfile2 >> u >>v)
        ...
        myfile2.close();
	size_t no_vertices = max + 1 ;//- min + 1;
        Graph g(no_vertices);
	//Populate Graph
		...
		{
{
				...
				if(owner(vertex(u,g)) == process_id(process_group(g))){
           	 		for (boost::tie(a_iter, a_end) = adjacent_vertices(vertex(u, g), g);
                	 		a_iter != a_end; ++a_iter)
            				{
           		  		   if( vertex(v, g) == *a_iter)
						flag = true;
	            			}
        	             	if(flag == false)
				{
				if(((((u+v)%2) == 0 ) && (u < v)) ||((((u+v)%2) == 1 ) && (u > v) )) 
                	     		add_edge(vertex(u,g),vertex(v,g),g);   //Keep
				}
		  	     }
		   }
{
				bool flag = false; 
            			a_iterator_type a_iter, a_end;
				if(owner(vertex(v,g)) == process_id(process_group(g))){
          	  		for (boost::tie(a_iter, a_end) = adjacent_vertices(vertex(v, g), g);
                	 		a_iter != a_end; ++a_iter)
            				{
           		  		   if( vertex(u, g) == *a_iter)
						flag = true;
            				}
					
        	             	if(flag == false)
				{
				if(((((u+v)%2) == 0 ) && (v < u)) || ((((u+v)%2) == 1 ) && (v > u) )) 
                	     			add_edge(vertex(v,g),vertex(u,g),g);   //Keep
				}
		  	}
		    }
  	...
  	if (process_id(process_group(g)) == 0) 
		std::cout<<process_id(process_group(g))<<"*******File Read Graph Created********\n";
